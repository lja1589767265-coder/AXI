...
Narrow Transfer（窄传输）表示 `2 ** AxSIZE` 小于数据总线字节数。lane 随地址选择，不能总把有效数据放在最低位。

| 总线 / 每拍大小 | INCR 地址序列 | 使用的 lane |
| --- | --- | --- |
| 32-bit / 1 Byte，5 拍 | `0、1、2、3、4` | `0、1、2、3、0` |
| 64-bit / 4 Byte，3 拍 | `4、8、12` | `4～7、0～3、4～7` |
| 64-bit / 4 Byte，WRAP 4 拍 | `4、8、12、0` | `4～7、0～3、4～7、0～3` |

INCR/WRAP 会按每拍地址选择 lane；FIXED 保持相同 lane 窗口。表中第三行按 16 字节回绕窗口计算，不能误以为 64 位总线就要求每拍 8 字节。
