...
新规范已统一采用“逐段全文中英双语对照”作为默认体例，格式以 A3 为准。A1、A3 已按该体例完成；A2 保留此前的“纯中文正文＋关键英文原句”版本，后续重制时也必须改为全文双语。A3 的旧版学习导读另行归档保留。

除用户明确指定其他体例外，翻译文档采用“逐段全文中英双语对照”的形式，章节和图表格式以 A3 为准。正文以适用版本的 Arm 官方规范为唯一内容依据，完整保留原文信息和出现顺序，不增加原文没有的教学讲解、案例、自测题、总结或结论。

本规范中的“全文翻译”是指：原文范围内每个具有实质内容的英文句子都必须有明确、完整的中文对应，不得只翻译关键规范句，也不得以章节摘要、段落概述或图表代替正文翻译。“关键英文原句”规则只限定哪些英文原文需要附在中文译文后，不缩小中文翻译的覆盖范围。
...
上传到微信公众号时，文章标题必须使用待上传 Markdown 文件的原文件名（含 `.md` 扩展名），不得使用文档内的一级标题替代。
...
1. 每个具有实质内容的原文句子都能在对应中文段落、列表项、提示块、公式、伪代码或中文重绘图表中找到一一对应的信息。
2. 原文中的普通叙述句、背景说明、原因、示例、条件和结果同样必须翻译，不能因为不需要附英文原句而省略。
3. 一个原文段落包含多个独立句子时，译文必须覆盖每个句子的全部信息；允许因中文语序在同一段落内调整表达，但不得压缩成只保留中心意思的摘要。
4. “全文翻译”与“全文双语”是两个不同要求：前者要求中文覆盖全部实质内容，后者还要求逐段保留全部英文原文。
...
1. 中英双语章节标题，保留原章节编号。
2. 来源元数据。
3. 按原文顺序排列的完整中文译文，并按第 5.5 节逐段保留完整英文原文。
4. 位于原文对应位置的中英双语重绘图表。
...
默认标题使用“中文 / English”形式，直接对应原文标题，不添加学习目标或读者收益。例如：

```text
# A5：Transaction 标识符 / Transaction Identifiers
```
...
- 每个原文段落对应一个中文段落，并逐句覆盖该段落的全部实质信息。
...
### 5.4 关键英文原句（仅适用于明确指定的非双语体例）

仅当用户明确指定“纯中文正文＋关键英文原句”体例时，正文以中文为主，并在对应中文段落后紧跟关键英文原句。满足以下任一条件时，必须附英文原句：
...
英文必须与 PDF 完全一致，不得节译、改写或拼接不同位置的句子。在该回退体例下，普通叙述句不重复附英文；默认的全文双语章节不适用本节的筛选规则。

### 5.5 全文双语章节（默认体例）

除用户明确指定其他体例外，所有新制作或重制章节均采用全文中英双语对照：
...
5. 原图图题和原表表题在图表旁保留完整英文原题名。
6. 不得使用英文摘要、解释或改写来替代原文。

A1、A3 已采用全文中英双语对照；A2 的现有版本是历史例外，重制时必须改为全文双语。任何新章节不得沿用第 5.4 节的回退体例，除非用户明确要求。
...
原文中的每张图和每个表都必须在原出现位置使用 draw.io 重绘。普通中文译稿使用中文标签；全文双语章节的原图重绘必须使用“中文 / English”双语标签。原文没有图或表时，不主动额外制作教学图表；用户明确要求的补充图按第 6.4 节处理。
...
6. 完成逐句覆盖检查，确认所有实质性英文句子均有中文对应；不得以关键英文原句代替中文翻译。
7. 默认全文双语体例逐段保留英文原文；只有用户明确指定第 5.4 节回退体例时，才按该节条件附上关键英文原句。
8. 使用 draw.io 重绘原文图表并导出 PNG。
9. 把图表引用插回原文对应位置。
10. 如果用户明确要求补充图，按第 6.4 节制作并插入指定位置。
11. 对照 PDF 和源内容清单逐项复核。
12. 完成结构、术语、图表和版本检查后更新 README 状态。
...
2. 每个具有实质内容的原文句子都有完整中文对应；普通叙述句与关键规范句采用相同的中文覆盖标准。
3. 没有遗漏、摘要、合并、重排或从其他章节提前引入内容。
4. 没有原文不存在且未经用户明确要求的学习目标、案例、总结、自测题或教学图。
5. 每个规范强度关键词的中文译法准确且一致。
6. 每条关键英文原句与 PDF 完全一致，并紧跟对应译文。
7. 所有信号名、字段名、编码、公式、数值和单位均未被误译。
8. 所有原图和原表均有同名 `.drawio` 与 PNG。
9. draw.io 源文件能够打开，PNG 来自最新源文件。
10. 图表编号、标题、内容、图例和脚注与原文一致。
11. 图片不存在裁切、重叠、乱码、黑块或不可读文字。
12. Markdown 只有一个一级标题，标题层级连续，图片路径存在。
13. 正文不存在 `<details>`、黄色结论高亮或外部教学链接。
14. 来源元数据中的文档版本、章节和页码正确。
15. 原始 PDF、旧学习稿和旧图未被修改。
16. `git diff --check` 无空白错误。
17. 补充图只包含相邻原文直接涉及的信息，没有自行扩展时序、行为、示例或结论。
18. 补充图未做中英双语对照；图已自解释时，正文没有重复说明。
19. 来源元数据及获准保留的非原文补充文字使用 `#D9822B`，原文译文和图内文字未被误着色。
20. 全文双语章节的每个中文正文段落或列表后都有完整英文原文块，图题和表题均保留完整英文原题名，图表内文字采用中英双语标签。
...
2. 原文中的每个实质性句子均有完整中文对应，不存在只翻关键句或用摘要代替全文的情况。
3. 原文顺序、结构、编号和规范强度保持不变。
4. 默认全文双语体例的英文原文块完整、准确；回退体例的关键英文原句数量适当且完整。
5. 术语在本章及已完成译文章节中一致。
6. 所有图表完成 draw.io 重绘和视觉核验。
7. 未加入原文不存在且未经用户明确要求的教学内容；获准添加的补充图符合第 6.4 节。
8. README 已更新翻译范围、图表数量、状态和待核对事项。
9. 全文双语章节已逐段核对英文原文块，并完成英文图题、表题和图表双语标签检查。

A1、A3 在 README 中标记为“全文双语稿”，A2 的历史版本标记为“忠实翻译稿”。新制作或重制的文档必须遵守默认全文双语规范。
