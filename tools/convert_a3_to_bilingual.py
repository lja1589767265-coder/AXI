"""One-time conversion of Chapter A3 from Chinese-full-text to A1-style bilingual Markdown."""
...
def english_block(source: str) -> str:
    lines = source.strip().splitlines()
    body = "\n".join(">" if not line else f"> {line}" for line in lines)
    return f"\n\n> **原文（English）**\n>\n{body}"


def caption(label: str, source: str) -> str:
...
def add_after(text: str, anchor: str, addition: str) -> str:
    ...
    while True:
        ...
        if index < 0:
            raise ValueError(f"No unused anchor: {anchor[:100]!r}")
...
def pair(anchor: str, source: str):
    return anchor, english_block(source)


def fig(anchor: str, source: str):
    return anchor, caption("原文图题", source)


def table(anchor: str, source: str):
    return anchor, caption("原文表题", source)


def convert():
    text = DOC.read_text(encoding="utf-8")
    if "/ Single Interface Requirements" in text:
        raise RuntimeError("A3 is already bilingual")

    # Remove the former key-sentence-only English quotations.
    text = re.sub(r"\n> 原文：[^\n]+(?:\n>\n> 原文：[^\n]+)*", "", text)

    headings = {
        "# A3：单接口要求": "# A3：单接口要求 / Single Interface Requirements",
        "## A3.1 时钟与复位": "## A3.1 时钟与复位 / Clock and reset",
        "### A3.1.1 时钟": "### A3.1.1 时钟 / Clock",
        "### A3.1.2 复位": "### A3.1.2 复位 / Reset",
        "## A3.2 基本读写 transaction": "## A3.2 基本读写 transaction / Basic read and write transactions",
        "### A3.2.1 握手过程": "### A3.2.1 握手过程 / Handshake process",
        "### A3.2.2 通道信号要求": "### A3.2.2 通道信号要求 / Channel signaling requirements",
        "#### 通道握手信号": "#### 通道握手信号 / Channel handshake signals",
        "#### 写地址通道": "#### 写地址通道 / Write address channel",
        "#### 写数据通道": "#### 写数据通道 / Write data channel",
        "#### 写响应通道": "#### 写响应通道 / Write response channel",
        "#### 读地址通道": "#### 读地址通道 / Read address channel",
        "#### 读数据通道": "#### 读数据通道 / Read data channel",
        "## A3.3 通道之间的关系": "## A3.3 通道之间的关系 / Relationships between the channels",
        "### A3.3.1 通道握手信号之间的依赖关系": "### A3.3.1 通道握手信号之间的依赖关系 / Dependencies between channel handshake signals",
        "#### 读 transaction 依赖关系": "#### 读 transaction 依赖关系 / Read transaction dependencies",
        "#### AXI3 写 transaction 依赖关系": "#### AXI3 写 transaction 依赖关系 / AXI3 write transaction dependencies",
        "#### AXI4 和 AXI5 写 transaction 依赖关系": "#### AXI4 和 AXI5 写 transaction 依赖关系 / AXI4 and AXI5 write transaction dependencies",
        "### A3.3.2 旧版兼容性考虑": "### A3.3.2 旧版兼容性考虑 / Legacy considerations",
        "## A3.4 Transaction 结构": "## A3.4 Transaction 结构 / Transaction structure",
        "### A3.4.1 地址结构": "### A3.4.1 地址结构 / Address structure",
        "#### 突发长度": "#### 突发长度 / Burst length",
        "#### 突发大小": "#### 突发大小 / Burst size",
        "#### 突发类型": "#### 突发类型 / Burst type",
        "#### 突发地址": "#### 突发地址 / Burst address",
        "### A3.4.2 传输的伪代码说明": "### A3.4.2 传输的伪代码说明 / Pseudocode description of the transfers",
        "### A3.4.3 Regular transaction": "### A3.4.3 Regular transaction / Regular transactions",
        "#### Regular transaction 属性": "#### Regular transaction 属性 / Regular transactions property",
        "#### 互操作性": "#### 互操作性 / Interoperability",
        "### A3.4.4 数据读写结构": "### A3.4.4 数据读写结构 / Data read and write structure",
        "#### 写选通": "#### 写选通 / Write strobes",
        "#### 窄传输": "#### 窄传输 / Narrow transfers",
        "#### 字节不变性": "#### 字节不变性 / Byte invariance",
        "#### 非对齐传输": "#### 非对齐传输 / Unaligned transfers",
        "### A3.4.5 读写响应结构": "### A3.4.5 读写响应结构 / Read and write response structure",
        "#### OKAY，普通访问成功": "#### OKAY，普通访问成功 / OKAY, normal access success",
        "#### EXOKAY，独占访问成功": "#### EXOKAY，独占访问成功 / EXOKAY, exclusive access success",
        "#### SLVERR，Slave 错误": "#### SLVERR，Slave 错误 / SLVERR, slave error",
        "#### DECERR，解码错误": "#### DECERR，解码错误 / DECERR, decode error",
    }
    for old, new in headings.items():
        if text.count(old) != 1:
            raise ValueError(f"Heading count mismatch: {old}")
    ...
    text = text.replace(
        "说明：正文按原文顺序逐段完整翻译，原文范围内所有实质性句子均有中文对应；仅在关键规范句后附英文原句。省略每页重复的页眉、页脚、版权行和物理页码。",
...
- All other signals can be driven to any value.""",
        ...
        fig("![图 A3-1：退出复位](image/axi-a3/figure-a3-1-exit-from-reset.png)", "Figure A3-1 Exit from reset"),
        pair(
            "- 通道信号要求，见第 A3-42 页。",
...
- The Channel signaling requirements on page A3-42""",
        ...
        fig("![图 A3-2：VALID 先于 READY 的握手](image/axi-a3/figure-a3-2-valid-before-ready-handshake.png)", "Figure A3-2 VALID before READY handshake"),
        pair(
        ...
        fig("![图 A3-3：READY 先于 VALID 的握手](image/axi-a3/figure-a3-3-ready-before-valid-handshake.png)", "Figure A3-3 READY before VALID handshake"),
        pair(
        ...
        fig("![图 A3-4：VALID 与 READY 同时握手](image/axi-a3/figure-a3-4-valid-with-ready-handshake.png)", "Figure A3-4 VALID with READY handshake"),
        pair(
...
- Read data channel on page A3-43""",
        ...
        table("![表 A3-1：Transaction 通道握手信号对](image/axi-a3/table-a3-1-transaction-channel-handshake-pairs.png)", "Table A3-1 Transaction channel handshake pairs"),
        pair(
...
- The master can assert RREADY before RVALID is asserted.""",
        ...
        fig("![图 A3-5：读 transaction 握手依赖关系](image/axi-a3/figure-a3-5-read-transaction-handshake-dependencies.png)", "Figure A3-5 Read transaction handshake dependencies"),
        pair(
...
- The master can assert BREADY before BVALID is asserted.""",
        ...
        fig("![图 A3-6：AXI3 写 transaction 握手依赖关系](image/axi-a3/figure-a3-6-axi3-write-transaction-handshake-dependencies.png)", "Figure A3-6 AXI3 write transaction handshake dependencies"),
        pair(
...
- The master can assert BREADY before BVALID is asserted.""",
        ...
        fig("![图 A3-7：AXI4 和 AXI5 写 transaction 握手依赖关系](image/axi-a3/figure-a3-7-axi4-axi5-write-transaction-handshake-dependencies.png)", "Figure A3-7 AXI4 and AXI5 write transaction handshake dependencies"),
        pair(
...
- AWSIZE[2:0], for write transfers""",
        ...
        table("![表 A3-2：突发大小编码](image/axi-a3/table-a3-2-burst-size-encoding.png)", "Table A3-2 Burst size encoding"),
        pair(
...
- AWBURST[1:0], for write transfers""",
        ...
        table("![表 A3-3：突发类型编码](image/axi-a3/table-a3-3-burst-type-encoding.png)", "Table A3-3 Burst type encoding"),
        pair(
...
- ACE5-LiteDVM""",
        ...
        table("![表 A3-4：Regular_Transactions_Only 互操作性](image/axi-a3/table-a3-4-regular-transactions-only-interoperability.png)", "Table A3-4 Regular_Transactions_Only Interoperability"),
        pair(
            "- 非对齐传输，见第 A3-56 页。",
...
- The burst type is INCR.""",
        ...
        fig("![图 A3-8：8 位传输的窄传输示例](image/axi-a3/figure-a3-8-narrow-transfer-8-bit.png)", "Figure A3-8 Narrow transfer example with 8-bit transfers"),
        pair(
            "- 传输在 64 位总线上进行。",
            """In Figure A3-9:
...
- The transfers are on a 64-bit bus.""",
        ...
        fig("![图 A3-9：32 位传输的窄传输示例](image/axi-a3/figure-a3-9-narrow-transfer-32-bit.png)", "Figure A3-9 Narrow transfer example with 32-bit transfers"),
        pair(
...
- The other data bytes are positioned in decreasing order of significance.""",
        ...
        fig("![图 A3-10：大端字节不变数据结构示例](image/axi-a3/figure-a3-10-big-endian-byte-invariant.png)", "Figure A3-10 Example big-endian byte-invariant data structure"),
        pair(
            "- 其他数据字节按有效性递增的顺序排列。",
...
- The other data bytes are positioned in increasing order of significance.""",
        ...
        fig("![图 A3-11：小端字节不变数据结构示例](image/axi-a3/figure-a3-11-little-endian-byte-invariant.png)", "Figure A3-11 Example little-endian byte-invariant data structure"),
        pair(
        ...
        fig("![图 A3-12：混合端序数据结构示例](image/axi-a3/figure-a3-12-mixed-endian-data-structure.png)", "Figure A3-12 Example mixed-endian data structure"),
        pair(
...
- Provide an aligned address and use the byte lane strobes to signal the unaligned start address.""",
        ...
        fig("![图 A3-13：32 位总线上的对齐与非对齐传输](image/axi-a3/figure-a3-13-aligned-unaligned-32-bit-bus.png)", "Figure A3-13 Aligned and unaligned transfers on a 32-bit bus"),
        pair(
        ...
        fig("![图 A3-14：64 位总线上的对齐与非对齐传输](image/axi-a3/figure-a3-14-aligned-unaligned-64-bit-bus.png)", "Figure A3-14 Aligned and unaligned transfers on a 64-bit bus"),
        pair(
        ...
        fig("![图 A3-15：64 位总线上的对齐回绕传输](image/axi-a3/figure-a3-15-aligned-wrapping-64-bit-bus.png)", "Figure A3-15 Aligned wrapping transfers on a 64-bit bus"),
        pair(
...
DECERR Decode error. Generated, typically by an interconnect component, to indicate that there is no slave at the transaction address. See DECERR, decode error on page A3-60.""",
        ...
        table("![表 A3-5：RRESP 和 BRESP 编码](image/axi-a3/table-a3-5-rresp-bresp-encoding.png)", "Table A3-5 RRESP and BRESP encoding"),
        pair(
...
- Access attempted to a disabled or powered-down function""",
    ...
    for anchor, addition in pairs:
        text = add_after(text, anchor, addition)

    DOC.write_text(text, encoding="utf-8")
    print(f"Converted {DOC.name} with {len(pairs)} bilingual content/caption blocks")
